...
let RefurbishedMobilesIndex = document.getElementById(
  "Refurbished-Mobiles-ind"
);
...
for (let checkbox of discountOpt) {
  checkbox.addEventListener("change", () => {
    ...
    filter = Array.from(discountOpt) // Convert checkboxes to an array to use filter and map.
...
for (let checkbox of PriceOpt) {
  checkbox.addEventListener("change", () => {
    let actualFilterData = [];
    filter = Array.from(PriceOpt) // Convert checkboxes to an array to use filter and map.
      .filter((i) => i.checked) // Use Array.filter to remove unchecked checkboxes.
      .map((i) => i.value); // Use Array.map to extract only the checkbox values from the array of objects.
...
for (let checkbox of RatingOpt) {
  checkbox.addEventListener("change", () => {
    let actualFilterData = [];
    filter = Array.from(RatingOpt) // Convert checkboxes to an array to use filter and map.
      .filter((i) => i.checked) // Use Array.filter to remove unchecked checkboxes.
      .map((i) => i.value); // Use Array.map to extract only the checkbox values from the array of objects.
...
for (let checkbox of BrandOpt) {
  checkbox.addEventListener("change", () => {
    let actualFilterData = [];
    filter = Array.from(BrandOpt) // Convert checkboxes to an array to use filter and map.
      .filter((i) => i.checked) // Use Array.filter to remove unchecked checkboxes.
      .map((i) => i.value); // Use Array.map to extract only the checkbox values from the array of objects.
...
for (let checkbox of ConditionOpt) {
  checkbox.addEventListener("change", () => {
    let actualFilterData = [];
    filter = Array.from(ConditionOpt) // Convert checkboxes to an array to use filter and map.
      .filter((i) => i.checked) // Use Array.filter to remove unchecked checkboxes.
      .map((i) => i.value); // Use Array.map to extract only the checkbox values from the array of objects.
...
function cards(
  ...
  let card = `
      ...
      <div class="card checkoutpagegone">
      <div class="img-div">
      <img src=${image} alt="">
      </div>
